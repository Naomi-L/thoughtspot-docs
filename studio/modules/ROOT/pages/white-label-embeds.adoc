...
an {product} report can be embedded into another website, such as a private wiki, blog, or any other website or application with access to the internet.
...
A link:https://mode.com/embedded-analytics/[White-Label Embed,window=_blank] (WLE) may be seen by viewers who are not logged in to {product} or members of your Workspace who do not have access to the underlying report.
...
* *No {product} viewer authentication:* With WLEs, the embed host application, instead of {product}, handles any required viewer authentication and programmatically controls access to the embedded reports by generating a unique, rapidly-expiring signed embed URL and (optionally) xref:parameters.adoc[parameters].
* *No {product} branding:* There is no {product} Analytics label in the embed border or loading animation.
* *Reduced viewer interactivity:* Some interactive features of {product}'s xref:visualizations.adoc#built-in-charts-and-tables[built-in charts and tables] are not available, including some drill-down features and chart-specific URLs.
...
The analytics in the portal are provided by two parameterized and White-Label Embedded {product} reports.
...
* The creator of the embedded report must be an active member of the same {product} Workspace that contains the report.

== Implementation steps

Embed an {product} report into a host application by following the steps below.
...
Otherwise, the most recent report run results from {product}'s servers will be returned.
* If the URL query string includes values for xref:parameters.adoc[parameters], {product} will return the most recent cached results using the same set of parameter values as long as those results are not older than the value of `max_age`.
If the report has never been run with the specified parameter values, {product} will run the report using them.
...
+
IMPORTANT: Your host application **must** create the `signature` using a process that is not transparent to embed viewers. Exposing your signature token's access secret in front-end, client-side code will compromise the security of your {product} account.
...
TIP: If you don't see the White-Label Embed tab, your Workspace does not have White-Label Embeds enabled. Contact your Workspace's {product} admins or {product} support to learn more about enabling White-Label Embeds.
...
You can also use {product}'s xref:internal-embeds.adoc#javascript [JavaScript embed helper library] to dynamically set the height.
...
For example, the following JavaScript listens for the embed response from {product}, retrieves the CSV download URLs, and assigns them as the targets of a couple of corresponding `<a>` elements:
...
Unlike CSV Export, where a click on the endpoint url itself would invoke a direct download, PDF Export requires a four-part process as described here in the link:https://mode.com/developer/api-cookbook/distribution/export-pdf/[{product} API cookbook,window=_blank].
...
=== Embedding reports built using the {product} Public Warehouse

Any {product} user can create an embed from a report built with data from the xref:managing-database-connections.adoc#mode-public-warehouse[{product} Public Warehouse], even if the report is in a Workspace that is not on a paid {product} plan.
However, the following limitations apply:

* The underlying report must be in a xref:spaces.adoc#community-space[community Collection] and therefore may *only* use data contained in the xref:managing-database-connections.adoc#mode-public-warehouse[{product} Public Warehouse].
...
Anyone can view an embedded report built using {product} Public Warehouse data when it is in a community Collection, even if they are not logged in to {product} or do not have an {product} account.
...
[discrete]
=== *Q: Why does my embed show an {product}-branded border?*

Only Internal Embeds include an {product}-branded border.
A White-Label Embed should not include an {product}-branded border.
If you are attempting to implement a White-Label Embed and the iframe includes an {product}-branded border when it is accessed, it is not implemented correctly and the embed is falling back to an Internal Embed when accessed.

To test this, attempt to access the embed while either logged out of {product} or using a private browser mode (for example, incognito mode in Google Chrome).
When you access the embed without an active {product} session, it should return an <<troubleshooting,error message>> that you can use to troubleshoot.

[#troubleshooting]
== Troubleshooting

Most issues with embeds stem from problems with generating the embed URL signature or other parts of the embed URL.
{product} offers a number of resources to help you test and troubleshoot White-Label Embeds:
...
White-Label Embed viewers may also receive this error when they try to view the embed but the underlying report's creator is no longer an active member of the {product} Workspace containing the report.
