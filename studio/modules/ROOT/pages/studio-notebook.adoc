...
For {product} Business and Enterprise paid plans, {product}'s Python notebook has 16GB of RAM and up to 16 CPU available to it.
...
NOTE: xref:studio-report-scheduling-and-sharing.adoc#scheduled-runs[Scheduled runs] will only re-run the Notebook if the Report view page contains at least one output generated by the Notebook. In this case, the Notebook will re-run as part of the scheduled report run.
...
| link:https://toblerity.org/fiona/[fiona] | 1.8.22 | 1.8.22 | read & write geospatial data files
...
| link:https://pypi.org/project/pandas-profiling/1.4.0/[pandas_profiling] | 3.5.0 | 3.5.0 | generates profile reports from a pandas DataFrame
...
We recommend using xref:studio-report-layout-and-presentation.adoc#text-boxes[Text Boxes] in our Report Builder.
