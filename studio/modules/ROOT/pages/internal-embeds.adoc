...
An Internal Embed built using your private data is only visible to logged-in members of your {product} Workspace who would otherwise have access to the underlying report.
...
Click the green *Powered by {product}* link at the bottom of the embed to open the underlying Report in {product}.

NOTE: Generally, you must be logged in to {product} and have access to the underlying Report in order to view an Internal Embed. However, the above embedded report is built on xref:managing-database-connections.adoc#mode-public-warehouse[{product}'s Public Warehouse] and is therefore viewable by anyone on the internet.

//Learn more about xref:white-label-embeds.adoc#embedding-reports-built-using-the-mode-public-warehouse[embedding reports built on {product}'s Public Warehouse].
...
Another option would be to grab code snippets including template code that you can customize and include in your host application that are generated in {product}.
...
You can also use {product}'s <<javascript,JavaScript embed helper library>> to dynamically set the height.
...
Internal Embeds feature a branded {product} border, which includes a *Filters* button in the lower right corner that you can use to toggle the xref:interactivity.adoc#filters-and-parameters[filter] panel open or closed.
...
If the following error returns, it's likely that your URL is incorrect or pointing at a Report that you may not have access to within {product}.

image::report_not_found.png[]

* Click on the green *Powered by {product}* link at the bottom of the embed to open the underlying Report in {product} to see if you have access.
If the report is visible in {product} and not within the embed, likely the URL is incorrect.
* If a user is unable to see the embed in the application, have them make sure to sign into {product} in another tab.
Only users that are signed in to {product} are able to view internal embeds.
* {blank}
